...
export default function PractitionerForumDetailPage() {
  ...
  useEffect(() => {
    const fetchForumData = async () => {
      try {
        ...
        const forumResponse = await fetch(`/api/practitioner/forums/${forumId}`);
        if (!forumResponse.ok) {
        ...
        const postsResponse = await fetch(`/api/practitioner/posts?forumId=${forumId}`);
        if (!postsResponse.ok) {
  ...
  const handlePostCreated = () => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch(`/api/practitioner/posts?forumId=${forumId}`);
        if (response.ok) {
  ...
  const currentForumColor = currentForum ? getForumColor(currentForum.id) : 'bg-blue-500';
  ...
  if (error || !currentForum) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-500 mb-4">Error: {error || 'Forum not found'}</p>
          <button 
  ...
  const transformedPosts = forumPosts.map(post => ({
    ...
    datePost: post.date_posted ? new Date(post.date_posted).toISOString() : new Date().toISOString(),
      ...
      <div className="sm:hidden">
        ...
        <div className="bg-card border-b border-border p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-10 h-10 rounded-full ${currentForumColor} flex items-center justify-center text-white text-lg font-bold`}>
        ...
        <main className="flex-1 p-6">
          <div className="mb-6">
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-16 h-16 rounded-full ${currentForumColor} flex items-center justify-center text-white text-2xl font-bold`}>
                {currentForum.topic.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1">
                <h1 className="text-3xl font-bold text-foreground">r/{currentForum.topic}</h1>
